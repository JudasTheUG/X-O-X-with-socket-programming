...
{
    public partial class ZamanModuLAN : Form
    {
        ...
        private void ZamanModuLAN_Load(object sender, EventArgs e)
        {
            MessageBox.Show("LAN bağlantısı ile onamak için gerekenler : " + Environment.NewLine + "1-)Kendi IP adresinizi 1.Oyuncunun IP adresi kısmına yazınız" +
        ...
        private void ZamanModuLAN_FormClosed(object sender, FormClosedEventArgs e)
        {
        ...
        {
            ZamanModuLAN game = new ZamanModuLAN();
            game.Show();
            this.Hide();
        ...
        private void MessageCallBack(IAsyncResult aResult)
        {
            ...
            if (size > 0)
            {
                ...
                string sure = receivedMessage[9].ToString();
                string gelenSıra = receivedMessage[10].ToString();

                netOynamaAktarma(gelenButton, gelenResim, sure, gelenSıra);
        ...
        public void Send(Button b, string xORo)
        {
            try
            {
                string s = b.Name;
                s += xORo;
                s += pbTime.Value.ToString();
                s += returner.ToString();
                System.Text.ASCIIEncoding enc = new System.Text.ASCIIEncoding();
                byte[] msg = new byte[1500];
                msg = enc.GetBytes(s); // x yada y değeri girilecek
                socket.Send(msg);
        ...
        public void netOynamaAktarma(string button, string resim, string sure, string gelenSıra)
        {
            ...
            for (int i = 0; i < buttons.Count(); i++)
            {
                if (button == buttons[i].Name)
                {
                    ...
                    if (resim == "X")
                    {
                        ...
                        turn = 2;
                        pbTime.Value = Convert.ToInt32(sure);
